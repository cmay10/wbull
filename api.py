...
class Api:
    ...
    def build_headers(self,access_token=None,trade_token=None,include_time=False,include_zone_var=False):
    ...
    def get_auth_tokens(self,username,password,mfa_code=None,security_question_id=None,answer=None):
        ...
        if security_question_id is not None and answer is not None:
            data['accessQuestions'] = '[{"questionId":"' + str(security_question_id) + '", "answer":"' + str(answer) + '"}]'
        ...
        if 'accessToken' in result.keys():
            ...
            dict = {k:v for k,v in result.items() if k in ['accessToken','refreshToken','tokenExpireTime']}
            dict['success'] = True
            return dict
    ...
    def refresh_tokens(self,access_token,refresh_token):
        ...
        response = requests.post(self._urls.refresh_tokens+refresh_token,json=data,headers=headers)
    ...
    def get_order_history(self,status='All',count=20):
        ...
        response = requests.get(self._urls.order_history(self._account_id, count) + str(status), headers=headers)
        return response.json()

    def get_ticker_id(self,ticker):
        headers = self.build_headers()

        params = {'keyword':ticker.upper(),
                'pageIndex':1,
                'pageSize':20,
                'regionId':self._region_code}
        if ticker and isinstance(ticker,str):
            response = requests.get(self._urls.ticker_id,headers=headers,params=params)
            result = response.json()
    ...
    def get_ohlc(self,ticker=None,ticker_id=None,interval='w1',count=100,extended_trading=0):
        ...
        df = pd.DataFrame(data,columns=['close_date','open','close','high','low','prev_close','volume','vwap'])
        df['close_date'] = pd.to_datetime(df.close_date,unit='s').dt.tz_localize('UTC').dt.tz_convert(timezone)
        df = df.replace('null',None)
        df = df.astype({'open':float,'close':float,'high':float,'low':float,'prev_close':float,'volume':float,'vwap':float})
    ...
    def place_single_order(self,ticker,action,quantity,order_type,time_in_force,extended_hours=False,lmt_price=None):
        headers = self.build_headers(access_token=self._access_token,trade_token=self._trade_token,include_time=True)

        data = {'action':action,'comboType':'NORMAL','orderType':order_type,'outsideRegularTradingHour':extended_hours,'quantity':round(quantity,5),'serialId':str(uuid.uuid4()),'tickerId':self.get_ticker_id(ticker),'timeInForce':time_in_force}
        ...
        response = requests.post(self._urls.place_single_order(self._account_id),json=data,headers=headers)
        result = response.json()

        return result

    def place_combo_order(self,ticker,action,quantity,order_type,time_in_force ,stop_loss=None,stop_gain=None,extended_hours = False):
        ...
        data = {
            'newOrders':[
                {'orderType':order_type,'timeInForce':time_in_force,'quantity':quantity,'outsideRegularTradingHour':extended_hours,'action':action,'tickerId':self.get_ticker_id(ticker),'comboType':'MASTER'},
            ]
        }
        ...
        if stop_loss:
            ...
            stop_order = {'orderType': 'STP', 'timeInForce': 'GTC', 'quantity': quantity,
                            'outsideRegularTradingHour': False, 'action': 'SELL', 'tickerId': self.get_ticker_id(ticker),
                            'auxPrice': stop_price, 'comboType': 'STOP_LOSS'}
            data['newOrders'].append(stop_order)

        if stop_gain:
            stop_price = round(quote * (1+stop_gain),2)
            print(quote,stop_price)
            stop_order = {'orderType': 'LMT', 'timeInForce': 'GTC', 'quantity': quantity,
                        'outsideRegularTradingHour': False, 'action': 'SELL', 'tickerId': self.get_ticker_id(ticker),
                        'lmtPrice': stop_price, 'comboType': 'STOP_PROFIT'}
            data['newOrders'].append(stop_order)
